...
def collect_rows_from_call(
    ...
    for agent_id, (ax, ay, s) in enumerate(agent_states_xyz):
        ...
        labels_k = _targets_to_labels(targets[agent_id], nodes_unique)
        # debug
...
def main():
    ...
    ap.add_argument("--stage", type=str, choices=["static", "dynamic"], default="dynamic",
                    help="stage=static 将强制 static 模式并默认仅采集首个快照，可配合 --static-full-episode 保持整局")
    ap.add_argument("--snapshot-only", action="store_true", help="每个 episode 仅记录第一次规划快照（静态训练阶段使用）")
    ap.add_argument("--static-full-episode", action="store_true",
                    help="stage=static 时收集完整 episode，直到所有需求完成且所有车辆回仓")
    args = ap.parse_args()
    ...
    static_full_episode = bool(args.static_full_episode)
    if stage == "static":
        mode = "static"
        if not static_full_episode:
            snapshot_only = True
