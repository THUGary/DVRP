...
def collate_rows(batch: List[Dict[str, Any]], max_targets: int | None = None) -> Dict[str, torch.Tensor]:
    ...
    - max_targets: optional limit on K used when padding/truncating labels
    """
    B = len(batch)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target_k", type=int, default=None, help="限制每个 agent 的目标序列长度 (<=0 表示使用数据集原始长度)")
...
def main():
    ...
    if args.stage == "dynamic" and args.freeze_base and adapter_dim_cfg <= 0:
        raise ValueError("--freeze_base requires adapter_dim > 0 so that adapter parameters exist for training.")
    ...
    collate_fn = functools.partial(collate_rows, max_targets=target_k)
    train_loader = DataLoader(trn_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, collate_fn=collate_fn)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, collate_fn=collate_fn)
    ...
    if args.stage == "dynamic":
        ...
        if args.freeze_base:
            model.freeze_base_and_train_adapters()
            total = _count_params(model.parameters())
            trainable = _count_params(p for p in model.parameters() if p.requires_grad)
            print(f"[INIT] adapter-only training enabled ({trainable}/{total} parameters trainable)")
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    if not trainable_params:
        raise RuntimeError("No trainable parameters found. Check --freeze_base and adapter settings.")
