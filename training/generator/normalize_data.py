...
# Robustly add project root to sys.path (search for configs.py upwards)
import pathlib
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mini', action='store_true', help='Use smaller settings matching mini dataset (auto-detect if path contains "mini")')
    args = parser.parse_args()

    # Auto mini if input path contains mini or flag provided
    if args.mini or 'mini' in args.input:
        print('[Normalize] Mini mode detected: expecting smaller time horizon and fewer rows per episode.')
