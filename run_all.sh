...
if [ ! -f "${CKPT_PATH}" ]; then
  echo "WARNING: expected checkpoint not found at ${CKPT_PATH}. Using the latest epoch file if exists."
  CKPT_PATH=$(ls -1t ${CKPT_DIR}/planner_${MAP_WID}_${AGENTS}_*.pt | head -n1)
fi
