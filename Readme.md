...
- 入口：`scripts/train_adversarial.py`
...
```bash
python scripts/train_adversarial.py --episodes 50 --device cuda --planner greedy --randomize_depot
```
...
现有脚本 `scripts/train_diffusion_generator.py` 和 `scripts/train_rl_diffusion_generator.py` 仍然可用；
建议逐步迁移到统一入口以减少重复代码与接口分歧。
## 快速运行（`run.py`）
规则基线：
...
scripts/
  ...
  train_rl_diffusion_generator.py  # 对抗式 RL 训练扩散生成器（使选定规划器奖励最小化）
agent/
...
2) 生成原始 CSV 数据（默认写入到 `scripts/generate_data.py` 内置的 DATASET_DIR，当前为绝对路径 `/data/ruled_generator`，请按需修改）：
```bash
...
从已有扩散监督权重初始化（提升稳定性）：
```bash
python scripts/train_rl_diffusion_generator.py --episodes 50 \
  --planner greedy \
...
启用实时渲染或保存帧：
```bash
python scripts/train_rl_diffusion_generator.py --episodes 5 --planner greedy --render --fps 8
python scripts/train_rl_diffusion_generator.py --episodes 5 --planner greedy --save_frames_dir adv_frames
```
机制要点：
...
从监督学习权重热启动，基于环境在线优化：
```bash
python train_rl_planner.py --episodes 200 --algo ppo --ckpt_init checkpoints/planner/planner_20_2_200.pt --save_best checkpoints/planner/planner_rl_best.pt --device cuda
```
...
3) `scripts/generate_data.py` 默认写入 `/data/ruled_generator`？— 这是演示路径，请修改脚本中的 `DATASET_DIR` 或在容器/主机创建对应目录。
