...
class Neighborhood:
    ...
    **require params:**
    ...
    def get_demands(self) -> List[Demand]:
        """Get all generated demands in this neighborhood"""
        return self.demands
    ...
    def _basic_demands(self, distribution: str, time_series: list[int]) -> List[Demand]:
        ...
        for i, (px, py) in enumerate(samples):
            ...
            end_t = self.max_time
            demand = Demand(x=px, y=py, t=0, c=c, end_t=end_t)
...
class StaticDemandGen(BaseDemandGenerator):
    ...
    def _initialize_neighborhoods(self) -> List[Neighborhood]:
        ...
        for _ in range(num_centers):
           # Sample center coordinates
            ...
            # For static demands, use max_time as the deadline
            # This ensures nodes don't expire before the episode ends
            env_params={
                "width":self.width,
                "height":self.height,
                "depot":self.depot,
                "max_time":self.max_time,  # Use max_time for static demands
            }
            burst_params={
    ...
    def sample(self, t: int) -> List[Demand]:
        ...
        for neighborhood in self.neighborhoods:
            demands = neighborhood.get_demands()
